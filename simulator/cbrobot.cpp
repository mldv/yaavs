...
void cbRobot::setSimulator(cbSimulator *s) {
    ...
    if (simulator->getScoring() == 0) {
        scorePenalties =
                100 + 100 * simulator->Lab()->nTargets(); //100 for each target area + 100 for returning COMPETITIVE
    } else {
...
void cbRobot::computeNextPosition() {
    ...
    double rot = (rightMotor.lastOutPower() - leftMotor.lastOutPower()) /
                 (2.0 * ROBOT_RADIUS); // outPower should be called only once per cycle
...
void cbRobot::updateScoreCompetitive() {
    ...
    switch (_state) {
        ...
        case RETURNING: {
            ...
            if (returningLed && receivedReturningLed() && targetAtPos() >= 0 &&
                visitedAllTargets()) { // robot restarting to return
                simulator->grAux->addFinalPoint(id, curPos.Coord());
                distAtArrival = simulator->grAux->dist(id);
                //cerr << simulator->curTime()<< ": R" << id << " distAtArrival=" << distAtArrival << "\n";
                startReturningTime = simulator->curTime();
                ...
                while (newCell.x != controlCellPath[0].x || newCell.y != controlCellPath[0].y) {
                    ...
                    for (d = -90; d <= 90; d += 90) {
                        if (simulator->Lab()->reachable(cbPoint(newCell.x * 2.0 + 1, newCell.y * 2.0 + 1),
                                                        cbPoint(newCell.x * 2 + 1 + cos((d + dir) * M_PI / 180.0) * 2.0,
                                                                newCell.y * 2 + 1 +
                                                                sin((d + dir) * M_PI / 180.0) * 2.0))) {
                    ...
                    if (d == 180) {
                        fprintf(stderr,
                                "Lab has no Loop! Does not fit for this challenge!\n"); // TODO: add Graphical Window Warning
            ...
            if (_state == RUNNING && simulator->curTime() == 0) {
                ...
                for (int cy = 0; cy < cells_height; cy++) {
                    for (int cx = 0; cx < cells_width; cx++) {
                        if (!simulator->Lab()->reachable(cbPoint(cx * 2.0 + 1.0, cy * 2.0 + 1.0),
                                                         cbPoint((cx + 1) * 2.0 + 1.0, cy * 2.0 + 1.0))) {
                            //fprintf(stderr,"not reachable %d %d -> %d %d, lmap %d %d\n", cy, cx, cy, cx+1,
                            //         (cy-initCell.y)*2+lmap_height/2,(cx-initCell.x)*2+1+lmap_width/2);
                            lmap[(cy - initCell.y) * 2 + lmap_height / 2][(cx - initCell.x) * 2 + 1 +
                                                                          lmap_width / 2] = '|';
                        }
                    }
                }
                ...
                for (int cy = 0; cy < cells_height; cy++) {
                    for (int cx = 0; cx < lmap_width; cx++) {
                        if (!simulator->Lab()->reachable(cbPoint(cx * 2.0 + 1.0, cy * 2.0 + 1.0),
                                                         cbPoint(cx * 2.0 + 1.0, (cy + 1) * 2.0 + 1.0))) {
                            lmap[(cy - initCell.y) * 2 + 1 + lmap_height / 2][(cx - initCell.x) * 2 +
                                                                              lmap_width / 2] = '-';
                        }
                    }
                }
        ...
        case STOPPED:
            ...
            if (_state == RUNNING && simulator->curTime() == 0) {
                ...
                for (unsigned int bi = 0; bi < simulator->Lab()->nBeacons(); bi++) {
                    ...
                    lmap[(targetCell.y - initCell.y) * 2 + lmap_height / 2][(targetCell.x - initCell.x) * 2 +
                                                                            lmap_width / 2] = '0' + bi;
                }
...
void cbRobot::updateStateLineControl2022() {
    switch (_state) {
        ...
        case STOPPED:
            ...
            if (_state == RUNNING && simulator->curTime() == 0) {
                controlCellPath[0].x =
                        curPos.X() + 0.5;   // THESE CELLS are 1x1 and centered on the possible line positions
                controlCellPath[0].y =
                        curPos.Y() + 0.5;   // this is different from cell concept used in updateStateControl (2021)
                struct cell_t newCell = controlCellPath[0];
                newCell.x++;
                nCellPath = 1;
                int dir = 0;
                while (newCell.x != controlCellPath[0].x || newCell.y != controlCellPath[0].y) {
                    ...
                    nCellPath++;
                    for (d = -90; d <= 90; d += 90) {
                        if (simulator->Lab()->isInside(cbPoint(newCell.x + cos((d + dir) * M_PI / 180.0) * 0.5,
                                                               newCell.y + sin((d + dir) * M_PI / 180.0) * 0.5))) {
                            newCell.x = round(cell.x + cos((dir + d) * M_PI / 180.0));
                            newCell.y = round(cell.y + sin((dir + d) * M_PI / 180.0));
                            dir = (dir + d + 360) % 360;
                ...
                for (int cy = 1; cy < cells_height - 1; cy++) {
                    for (int cx = 1; cx < cells_width; cx++) {
                        if (simulator->Lab()->isInside(cbPoint(cx * 2.0, cy * 2.0 + 0.5))) {
                            //fprintf(stderr,"not reachable %d %d -> %d %d, lmap %d %d\n", cy, cx, cy, cx+1,
                            //         (cy-initCell.y)*2+lmap_height/2,(cx-initCell.x)*2+1+lmap_width/2);
                            lmap[(cy - initCell.y) * 2 + lmap_height / 2 + 1][(cx - initCell.x) * 2 +
                                                                              lmap_width / 2] = '|';
                        }
                    }
                }
    ...
    switch (_state) {
        ...
        case WAITOTHERS: {
            if (distGridToBeacon == 0.0) {
                simulator->grAux->addFinalPoint(id, getSimulator()->Lab()->Beacon(
                        0)->Center()); // VALID ONLY FOR ONE BEACON MAZES
                distGridToBeacon = simulator->grAux->dist(id);
            }

            if (distHomeToTarget == 0.0) {
                simulator->grAux->addFinalPoint(1, getSimulator()->Lab()->Beacon(
                        0)->Center()); // VALID ONLY FOR ONE BEACON MAZES
            ...
            if (visitingLedOn() && targetAtPos() == 0 && _state == RUNNING) { // robot visiting Target Area
                ...
                if (simulator->curTime() < simulator->keyTime()
                    ...
                    scorePenalties -= targetReward / 2 +
                                      targetReward / 2 * (simulator->simTime() - simulator->curTime()) /
                                      (simulator->simTime() - simulator->keyTime());
...
//    }
    ...
    if (showMeasures)
        simulator->GUI()->writeOnBoard(
                "Measures sent to " + QString(name) + "(robot " + QString::number(id) + ")" + ":\n" + xml, (int) id, 0);
